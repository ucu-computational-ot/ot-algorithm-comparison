...
@dash.callback(
    Output("desc-maxiter-per-epsilon-heatmap", "figure"),
    *FILTER_INPUTS,
)
def update_maxiter_heatmap_vs_reg(solvers, regs, dims, datasets, size):
    df = get_filtered(solvers, regs, dims, datasets, size)
    required = {"solver", "distribution", "reg", "iterations", "maxiter"}
    if df.empty or not required.issubset(df.columns):
        fig = go.Figure()
        fig.update_layout(
            title="Max-iter hit rate vs regularization",
    ...
    df["maxiter"]    = pd.to_numeric(df["maxiter"], errors="coerce")
    df = df[~df["reg"].isna()]  # need reg on x

    # --- stats: max-iter hit rate per (dataset, reg, solver) + counts ---
    keys = ["distribution", "reg", "solver"]

    total_runs = df.groupby(keys, dropna=False).size().rename("total_runs")
    maxiter_stats = (
        df.assign(is_maxiter_hit=(df["iterations"] >= df["maxiter"]))
          .groupby(keys, dropna=False)["is_maxiter_hit"]
          .sum()
    ...
    reg_vals   = np.sort(maxiter_stats["reg"].dropna().unique())
    col_labels = [str(v) for v in reg_vals]
    ...
    for i, sol in enumerate(solver_list):
        ...
        d = maxiter_stats[maxiter_stats["solver"] == sol].copy()
