...
from .utils import QuadraticProgrammingProblem, TerminationStatus, TimingData, OTProblem, PrimalShape
...
def pdhg_final_log(
    ...
    if logging.root.level <= logging.INFO:
        # Print primal norms
        jax_debug_log(
            "  primal norms: L2={:15.10g}",
            jnp.linalg.norm(avg_primal_solution.P),
            logger=logger,
            level=logging.INFO,
        )
        # Print dual norms
        jax_debug_log(
            "  dual norms: L2={:15.10g}",
