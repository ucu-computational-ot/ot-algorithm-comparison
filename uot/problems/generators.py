...
class ProblemGenerator(ABC):
    ...
    def generate(self, *args, **kwargs) -> List:
        ...
        If problems are generated but not loaded they are dumped 
        """
    ...
    def _generate(self) -> list[ArrayLike]:
        """
        Generate problems
        """

        pdfs_num = 2 * self._num_problems
        axes_support = self._get_axes(self._n_points)
        points = generate_nd_grid(axes_support)
        mixtures_pdfs = self._generate_pdfs(pdfs_num=pdfs_num)

        problems: List[TwoMarginalProblem] = []
        for i in range(self._num_problems):
            mu = DiscreteMeasure(
                points=points,
    ...
    @abstractmethod
    def _generate_pdfs(self, pdfs_num):
        """
        Generate pdfs for problems
        """
        pass
    ...
    def _load_problems(self) -> list[MarginalProblem] | None:
        ...
        for filename in os.listdir(ProblemGenerator.DUMP_PATH):
            ...
            # If this generator was serialized, load its problems
            if generator == self:
                return problems
    ...
    def _dump_problems(self, problems: list[MarginalProblem]):
        """
        Dump generated problems with generator instance 
        """
